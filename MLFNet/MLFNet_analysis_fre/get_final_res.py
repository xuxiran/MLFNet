...
parser.add_argument('--fre', type=int, default=0,choices=[0,1,2,3,4,5])
...
for sbfold in range(4):
    csvname = savedir + dataset_name + '_' + model_name + '_' + str(sbfold)+ '_' + str(args.fre) + '.csv'
...
if not os.path.exists(res_dir):
    os.makedirs(res_dir)
np.savetxt('./model_result/' + dataset_name+'_'+model_name+ '_' + str(args.fre)+'.csv', res.numpy(), delimiter=',')
