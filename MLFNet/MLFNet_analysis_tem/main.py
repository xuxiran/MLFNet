...
parser.add_argument('--tems', type=int, default=0,choices=[0,125,250,375,500,625,750,875])
parser.add_argument('--teme', type=int, default=1000, choices=[125,250,375,500,625,750,875,1000])
...
if __name__ == '__main__':
    ...
    mask = torch.zeros(5, 128, 10, 11)
    mask_1 = torch.ones(5, 128, 10, 11)
    sample_start = args.tems*128//1000
    sample_end = args.teme*128//1000
    mask[:,sample_start:sample_end,:,:] = mask_1[:,sample_start:sample_end,:,:]
    ...
    if args.only_evaluate == 0:
        ...
        for epoch in range(epoch_num):
            ...
            if decoding_acc > decoding_acc_final:
                ...
                saveckpt = savedir + '/fold' + str(fold) + '_' + str(args.tems)+ '_' + str(args.teme) + '.ckpt'
    ...
    if not os.path.exists(savedir):
        os.makedirs(savedir)
    np.savetxt(savedir + dataset_name+'_'+model_name+'_'+str(sbfold)+ '_' + str(args.tems)+ '_' + str(args.teme)+'.csv', res.numpy(), delimiter=',')
